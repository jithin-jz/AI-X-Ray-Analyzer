...
from backend.services.webauthn_service import RP_ID, RP_NAME, ORIGIN, challenge_store
...
async def passkey_register_start(email: str, db=Depends(get_db)):
    ...
    if not user:
        raise HTTPException(status_code=404, detail="User not found. Register with password first.")
...
async def passkey_register_verify(email: str, response: dict = Body(...), db=Depends(get_db)):
    ...
    scrambled_password = bcrypt.hashpw(random_master_key.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
...
async def passkey_login_verify(email: str, response: dict = Body(...), db=Depends(get_db)):
    user = await db.users.find_one({"email": email})
    if not user or not user.get("credential_id"):
        raise HTTPException(status_code=400, detail="No passkey found")
